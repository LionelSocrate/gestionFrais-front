...
export default function UpdatePaiement({ handleAfficheUpdate, id, numMatr }) {
  ...
  const handleSubmit = (e) => {
    ...
    try {
      const newPaiement = async () => {
        await fetch(`http://localhost:5000/api/paiement/updatePaiement/${id}`, {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            numMatr: numMatrUpdate,
            frais: frais,
          }),
        }).then((res) => res.json());
      };
          ...
          <div className=" flex flex-col gap-8">
            <div className=" w-full flex flex-col gap-3">
              ...
              <input
                ...
                onChange={(e) => setNumMatrUpdate(e.target.value)}
              />
            </div>
            <div className=" w-full flex flex-col gap-3">
